...
Agent Company is a repo-local Codex plugin that lets the current Codex session hand user goals to a persistent tmux CEO agent, which then coordinates persistent Codex TUI workers through a TypeScript MCP server.

## Components

- `skills/company/SKILL.md` defines the gateway-to-CEO workflow.
...
Important product, architecture, UX, release, and major scope judgments go through the CEO first. The CEO delegates focused written review only to the roles whose ownership is involved, then persists meaningful outcomes as meeting notes, decisions, or discussion records when an explicit discussion artifact is useful.
...
Version `0.1.1` can remain stale in `~/.codex/plugins/cache/agentinc-local/agent-company/0.1.1` with the older `Content-Length` framed MCP server. Version `0.1.2` can remain stale without automatic dashboard startup, version `0.1.3` can remain stale without worker stall failure recording, version `0.1.4` can remain stale without owner-role routing rules, version `0.1.5` can remain stale without the dashboard startup/reporting guidance and LAN URL metadata, version `0.1.6` can remain stale without the long-running implementation wait guidance, and version `0.1.7` can remain stale without the persistent CEO entrypoint. If `codex mcp get agent-company` points at an older cache version, reinstall the local plugin so the active MCP cwd points at `0.1.8`, then start a new Codex session before checking for Agent Company tools.

## Safety

The CEO may delegate planning, implementation, QA, and release preparation. Deployment, destructive deletion, cost-incurring actions, external publication, credential changes, and major direction changes require explicit user approval.
