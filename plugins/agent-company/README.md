...
Version `0.1.1` can remain stale in `~/.codex/plugins/cache/agentinc-local/agent-company/0.1.1` with the older `Content-Length` framed MCP server. Version `0.1.2` can remain stale without automatic dashboard startup, version `0.1.3` can remain stale without worker stall failure recording, version `0.1.4` can remain stale without owner-role routing rules, version `0.1.5` can remain stale without the dashboard startup/reporting guidance and LAN URL metadata, and version `0.1.6` can remain stale without the long-running implementation wait guidance. If `codex mcp get agent-company` points at an older cache version, reinstall the local plugin so the active MCP cwd points at `0.1.7`, then start a new Codex session before checking for Agent Company tools.
