...
2. 초기 입장 라운드. 각 직원이 역할 관점의 판단, 근거, 리스크, 필요한 질문을 `statement`로 남긴다.
3. 상호 반박 라운드. 각 직원이 최신 메시지를 읽고 다른 참가자의 메시지 sequence 또는 id를 참조한 `reply`를 남긴다.
4. 입장 수정 라운드. 각 직원이 자기 입장이 바뀌었는지와 그 이유를 남긴다.
5. 최종 합의 라운드. 각 직원이 `consensus` 메시지에 입장, 근거, 참조한 메시지, 조건 또는 사용자 질문을 남긴다.

참가자가 한 명뿐인 회의는 상호 반박 라운드를 생략할 수 있다. 여러 참가자가 있는 회의에서 다른 직원 발언을 참조하지 않은 최종 `agree`는 충분한 합의 근거로 보지 않는다.
